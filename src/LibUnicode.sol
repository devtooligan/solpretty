...
/// @author Devin Stein -- Minor modifications by devtooligan
library Unicode {
  /// @notice Check if `self` contains only single byte ASCII characters (0-127)
  /// @dev If a string is only ASCII, then it's safe to treat each byte as a character. This returns false for extended ASCII (128-255) because they use two bytes in UTF-8.
  /// @param self The input string
  /// @return True if  the `self` only contains ASCII
  function isASCII(string memory self) internal pure returns (bool) {
  ...
  function isUTF8(string memory self) internal pure returns (bool) {
    bytes memory _b = bytes(self);
    uint256 end = _b.length;
    uint32 cp;
    uint256 i;
  ...
  function decodeChar(string memory self, uint256 _cursor)
    public
    pure
  ...
  function decode(string memory self)
    internal
    pure
  ...
  function charAt(string memory self, uint256 _idx)
    public
    pure
    returns (string memory)
  {
  ...
  function codePointAt(string memory self, uint256 _idx)
    internal
    pure
    returns (uint32)
  {
  ...
  function indexOf(string memory self, string memory _of)
    internal
    pure
    returns (uint256)
  {
  ...
  function bytesIndicesOf(string memory self, string memory _of)
    internal
    pure
